...
interface CommissionRulesTabProps {
    commissionerId?: string  // Optional: filter rules for a specific commissioner
}
...
export function CommissionRulesTab({ commissionerId }: CommissionRulesTabProps = {}) {
    ...
    const fetchRules = async () => {
        ...
        // Filter by commissioner if commissionerId is provided
        if (commissionerId) {
                ...
                <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                        <thead style={{ background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                            <tr>
                                {!commissionerId && <th style={{ padding: '0.75rem 1rem', fontSize: '0.875rem', fontWeight: 600, color: '#64748b' }}>Comercial</th>}
                            ...
                            {rules.length === 0 ? (
                                ...
                                rules.map((rule: any) => (
                                    <tr key={rule.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                                        {!commissionerId && <td style={{ padding: '0.75rem 1rem' }}>{rule.commissioners?.full_name || 'Desconocido'}</td>}
