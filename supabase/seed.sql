-- Seed Data for EnergyDeal CRM
-- Market Tariffs (Endesa, Iberdrola, Naturgy)
-- Initial Commission Rules

BEGIN;

-- Variables locally aren't supported in standard SQL migrations as easily as in scripts,
-- but we can use subqueries. We assume the current company_id is the one we created.
-- For this seed to work globally, we usually need a function or to do it after login.
-- However, for the USER's cloud project, we can try to guess or use the first company.
